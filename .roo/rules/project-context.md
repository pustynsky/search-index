...
- **Keep policyReminder adaptive; never remove reinforcement blindly** — modern models still drift from Xray routing, but repeating the same policy on every response creates avoidable context. Default cadence is first response, every 25 responses, and after 30 minutes idle; dynamic `nextStepHint` and warnings remain continuous. `XRAY_POLICY_REMINDER=always` preserves compatibility for older models/clients and `off` is explicit opt-out. A local transcript sample showed substantial removable fixed policy framing while retaining actionable hints.
