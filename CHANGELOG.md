...
- **`xray_edit` now takes an explicit `lineEnding` parameter (`preserve` | `lf` | `crlf`).** Previously a new file was always written with LF and there was no way to ask for CRLF at all — passing `\r\n` inside `content` did not help, because input is parsed into logical lines and re-serialized with the format the tool picked. Existing behavior is unchanged when the parameter is omitted: an existing file keeps its detected format, a new file is still LF. `preserve` is rejected for a file that does not exist yet. Converting an existing file counts as a real change — the file is rewritten, `resultHash` differs from `sourceHash`, and the diff reads `(line endings: LF -> CRLF)` when the text itself is unchanged; a conversion with no text edit is `operations: []` plus `lineEnding`. Responses (including `dryRun`) add `requestedLineEnding`, `originalLineEnding`, `resultLineEnding`, `lineEndingDecisionSource` (`explicit` | `preserve` | `default-preserve` | `fallback-lf`) and `lineEndingChanged`, with `lineEnding` kept as an alias of `resultLineEnding`. The parameter is resolved per file for `paths`, mixed LF/CRLF files are still rejected before any write, and a file with no line terminators — byte-identical under either format — reports `originalLineEnding: null` and `lineEndingChanged: false` instead of claiming a conversion.
