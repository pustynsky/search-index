...
- **Added test-only lexical scorer matrices and completed a locked BM25 comparison.** Candidate and quality runners can evaluate TF-IDF, smoothed TF-IDF, and BM25 configurations without changing production behavior. The tuning-corpus-selected `bm25-file-stem-v2-k1-2-b-0.5` challenger improved aggregate NDCG@10 on both the tuning corpus and a separate private holdout, but materially regressed `multi_term_and` ranking on both and reduced holdout MRR@10, so production remains on `tfidf-file-stem-v1`.
