...
fn handle_search_help() -> ToolCallResult {
    let help = json!({
        "bestPractices": [
            ...
            {
                "rule": "Multi-term OR search: find all variants of a class in ONE query",
                "why": "Comma-separated terms with mode='or' finds files containing ANY term. Much faster than making separate queries for each variant.",
                "example": "search_grep with terms='UserService,IUserService,UserServiceFactory', mode='or'"
            },
            {
                "rule": "Multi-term AND search: find files containing ALL terms",
                "why": "mode='and' finds files where ALL comma-separated terms co-occur. Useful for finding DI registrations or wiring files.",
                "example": "search_grep with terms='ServiceProvider,IUserService', mode='and'"
            },
            {
            ...
            {
                "rule": "Phrase search: use phrase=true for exact multi-word match",
                "why": "phrase=true finds exact adjacent word sequences (slower ~80ms but precise). Useful for 'new HttpClient', 'throw new ArgumentException', etc.",
                "example": "search_grep with terms='new HttpClient', phrase=true"
            },
            {
                "rule": "Regex pattern search: use regex=true",
                "why": "Full regex support for pattern matching across all tokens. Useful for finding all interfaces matching a pattern.",
                "example": "search_grep with terms='I[A-Z]\\w+Cache', regex=true. Also works in search_definitions: name='I.*Service', regex=true"
            },
            {
                ...
                "example": "excludeDir=['test','Mock','ComponentTests','UnitTests'] — works in search_grep, search_definitions, search_callers"
            },
            {
                "rule": "Call chain tracing: use search_callers (up and down)",
                "why": "Single sub-millisecond request replaces 7+ sequential search_grep + read_file calls. direction='up' finds who calls this method (default). direction='down' finds what this method calls.",
                "example": "search_callers with method='GetUserAsync', class='UserService', depth=2, direction='up' (or 'down')"
            },
            {
            ...
            {
                "rule": "Stack trace analysis: use containsLine",
                "why": "Given a file + line number, returns the innermost containing method AND its parent class. Perfect for understanding stack traces without manually reading files.",
                "example": "search_definitions with file='UserService.cs', containsLine=42"
            },
            {
                "rule": "Read method source: use search_definitions with includeBody=true",
                "why": "Returns method body inline, eliminating read_file round-trips. Use maxBodyLines/maxTotalBodyLines for budget control. Set maxBodyLines=0 for unlimited.",
                "example": "search_definitions with parent='UserService', includeBody=true, maxBodyLines=20, maxTotalBodyLines=100"
            },
            {
                "rule": "Body budget: maxBodyLines=0 and maxTotalBodyLines=0 mean unlimited",
                "why": "Default limits are 100 lines per definition and 500 total. Set to 0 for unlimited output (large responses). Use defaults for typical queries.",
                "example": "search_definitions with parent='UserService', includeBody=true, maxBodyLines=0, maxTotalBodyLines=0 — returns ALL source code"
            },
            {
        ...
        "toolPriority": [
            "1. search_callers — call trees, up AND down (<1ms)",
            "2. search_definitions — structural: classes, methods, interfaces, containsLine (<1ms for baseType/attribute)",
            "3. search_grep — content: exact/OR/AND (<1ms), substring (~1ms), phrase (~80ms), regex (~60ms)",
