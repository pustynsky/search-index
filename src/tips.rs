...
pub fn parameter_examples(def_extensions: &[String]) -> Value {
    ...
    json!({
        ...
        "xray_callers": {
            ...
            "includeGrepReferences": "includeGrepReferences=true -> adds grepReferences[] with files containing the method name as text but NOT in the call tree. Catches delegate usage, method groups, reflection. An in-scope definition-indexed reference sets resultStatus.safeForExhaustiveClaims=false with reason grep_references_outside_call_graph. Test refs remain advisory only with productionOnly=true; excluded and text-only refs remain advisory. Skipped for method names < 4 chars. Each entry has file + tokenCount. For line-level detail, use xray_grep with showLines=true"
        },
        "xray_fast": {
        ...
        "xray_edit": {
            ...
            "lineEnding": "'preserve' | 'auto' | 'lf' | 'crlf'. Default: 'preserve' for an existing file, 'auto' for a new one. 'preserve' errors on a file that does not exist yet. 'auto' equals 'preserve' for an existing file; for a new file it follows the line ending Git would materialize in the working tree, resolved ONLY through Git: eol attribute -> text/text=auto plus core.autocrlf -> core.eol -> platform default. An unspecified text attribute with core.autocrlf=false, a '-text' attribute, no worktree, a missing Git, or any Git failure all resolve to LF. The legacy crlf attribute is honoured per the gitattributes(5) table (crlf -> text, -crlf -> -text, crlf=input -> eol=lf), but only when neither text nor eol applies — git check-attr text eol does not surface it. That is what lets a created file pass 'git add' under core.safecrlf. No Git process is spawned when there is no .git above the path, and any Git failure degrades to LF. \\n or \\r\\n inside 'content' never overrides the choice — input is always parsed into logical lines. Converting an existing file is a real change: writeStatus='committed', resultHash != sourceHash, diff reads '(line endings: LF -> CRLF)'. Response adds requestedLineEnding, originalLineEnding, resultLineEnding, lineEndingDecisionSource ('explicit' | 'preserve' | 'default-preserve' | 'gitattributes-eol' | 'git-worktree-policy' | 'fallback-lf'), lineEndingChanged; 'lineEnding' stays as an alias of resultLineEnding. Applied per file for 'paths'. Pure conversion with no text edit: operations=[] + lineEnding. A file with no line terminators is byte-identical under either format — it reports originalLineEnding=null and lineEndingChanged=false.",
