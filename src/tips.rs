...
pub fn parameter_examples(def_extensions: &[String]) -> Value {
    ...
    json!({
        ...
        "xray_edit": {
            ...
            "lineEnding": "'preserve' | 'lf' | 'crlf'. Default: 'preserve' for an existing file, LF for a new one. 'preserve' errors on a file that does not exist yet. \\n or \\r\\n inside 'content' never overrides it — input is always parsed into logical lines. Converting an existing file is a real change: writeStatus='committed', resultHash != sourceHash, diff reads '(line endings: LF -> CRLF)'. Response adds requestedLineEnding, originalLineEnding, resultLineEnding, lineEndingDecisionSource ('explicit' | 'preserve' | 'default-preserve' | 'fallback-lf'), lineEndingChanged; 'lineEnding' stays as an alias of resultLineEnding. Applied per file for 'paths'. Pure conversion with no text edit: operations=[] + lineEnding. A file with no line terminators is byte-identical under either format — it reports originalLineEnding=null and lineEndingChanged=false.",
