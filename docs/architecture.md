...
- **Component tree navigation via `xray_callers`:** for single-method queries, `direction='down'` accepts a component class or selector and shows child selectors; `direction='up'` accepts a selector or component class and uses `template_parents` without scanning every component. `productionOnly`, `excludeDir`, `excludeFile`, and `ext` filter roots and recursive nodes before ambiguity, totals, and pagination. Multiple in-scope class or downward-selector roots return `ambiguous_template_root`; `query.templateRootInterpretation` reports whether class or selector precedence was used. Explicit `ambiguityPolicy='legacy'` preserves root fan-out and sets `summary.legacyAmbiguousRootFanOut=true`. Dynamic or missing selector/template states, unavailable external templates, and unresolved child selectors make `resultStatus` partial and non-exhaustive.

  `summary.templateResolutionReasons` / `resultStatus.reasons` use: `ambiguous_template_root`, `template_root_out_of_scope`, `ambiguous_template_selector`, `unresolved_template_selector`, `stale_template_selector_index`, `stale_template_definition`, `dynamic_template_index_blind_spot`, `component_record_unavailable`, `dynamic_component_selector`, `missing_component_selector`, `dynamic_template`, `missing_template`, and `unavailable_external_template`.
- **HTML content search:** add `html` to `--ext` / `ext` for `xray_grep` to search external template content.

**Limitations.** Only active-context custom tags with hyphens are indexed; comments, declarations, CDATA, quoted attributes, and `script`/`style` raw text are skipped, and `ng-*` tags are excluded. Templates larger than the shared 4 MiB parser limit are skipped.
