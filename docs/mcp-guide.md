...
Angular `@Component` metadata is parsed from the TypeScript AST. Static inline and in-workspace external templates share one tokenizer and one component graph; persisted `template_owners` records external ownership for lifecycle work, while `template_parents` powers direct reverse lookup. For single-method queries, template navigation accepts class names and selectors, reports the chosen precedence in `query.templateRootInterpretation`, and applies `productionOnly`, `excludeDir`, `excludeFile`, and `ext` before root ambiguity and recursive traversal. Multiple in-scope class or downward-selector roots return `ambiguous_template_root`; fully filtered roots report `template_root_out_of_scope` unless the same text resolves as a selector, in which case `templateRootInterpretation="selector"` reports the fallback. Explicit `ambiguityPolicy="legacy"` preserves root fan-out and sets `summary.legacyAmbiguousRootFanOut=true`. Duplicate child selectors return `ambiguous_template_selector` candidates without choosing a class. Dynamic or missing selector/template states, unavailable external templates, plus temporarily unavailable records during incremental reconciliation, appear in `summary.templateResolutionReasons` and make `resultStatus` partial and unsafe for exhaustive claims.
...
| `ambiguityPolicy`    | C# and TypeScript root ambiguity behavior: `"report"` (default) returns candidates without traversing ambiguous roots; `"legacy"` preserves historical fan-out and marks the result partial and unsafe for exact semantics. |
...
If one single-method caller root is too large even after body stripping, the response switches to `resultStatus.page.unit="callTreeNodes"`. Each `callTreeNodes` record contains `nodeId`, `parentNodeId`, `relation`, `ordinal`, and `node` (the original node without child arrays). Reconstruct the tree by attaching each record to `parentNodeId` under `relation` in `ordinal` order. Continue with the same token loop; after the subtree finishes, the cursor advances to the next top-level root automatically. Node pages are graph-only: bulky auxiliary fields such as `rootMethod`, `testsCovering`, and `grepReferences` are omitted and listed in `summary.nodePageOmittedFields`. Oversized multi-method batch entries still return the bounded single-item error; page those methods individually when subtree continuation is required.
...
| `defIndexUpdated` | `bool` | Always (single-file) / per-file (multi-file) | `true` only after the complete definition delta was applied. With `--definitions`, this includes owned external Angular HTML even when `html` is absent from `--ext`; pending/raced enrichment and skipped files return `false` |
| `definitionGeneration` | `u64` | Single-file / `summary` for multi-file when definitions are enabled | Persisted definition generation observed after the synchronous attempt; it advances only after a complete delta apply |
...
| `"extensionNotIndexed"` | The file's extension is not in `--ext` (case-insensitive) and it is not an owned Angular template | The server only indexes configured content extensions; owned external Angular HTML is the narrow exception because it is an auxiliary definition input |
...
**Multi-file batching:** `handle_multi_file_edit` collects all eligible files and calls `reindex_paths_sync` exactly once after the Phase 3b rename step. Definition results are tracked per path: a stable file can report `completed` / `defIndexUpdated: true` while a concurrently busy peer reports `pending` / `false`. This also keeps the inverted-index write lock to one batched acquisition instead of N calls.

**Concurrent safety:** `reindex_paths_sync` reuses the watcher's non-blocking `RwLock` pattern. Source parsing and Angular template I/O/tokenization happen outside the write lock; `(size, mtime, SHA-256)` fingerprints are revalidated in parallel before one atomic stable-subset apply. Raced inputs and TypeScript sources that depend on raced templates are retried once, while unrelated stable peers apply immediately. A second race returns `reindexStatus: "pending"`, `defIndexUpdated: false`, and `reindexWarning` for only the deferred paths instead of claiming completion. Transient input errors are also tracked per path, and a persistent error converges through path-aware tombstoning after three failed batches. A dedicated stress test (`test_sync_reindex_concurrent_edit_and_grep_no_deadlock` in `src/mcp/handlers/edit_tests.rs`) verifies that 20 parallel `xray_edit` calls do not deadlock against continuous `xray_grep` reads on the same file (5-second deadline).
