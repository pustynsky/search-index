...
### Frozen hybrid follow-up (2026-07-31)

The class-aware rule was frozen before candidate collection and evaluated once on a third, independent private machine-local 301-file TypeScript holdout with 35 lexical queries: token-OR, substring-OR, and substring-AND only, with no regex, phrase, or line-regex requests. The TF-IDF/hybrid candidate union contained 1,013 query-path pairs: 159 retrieved judgments and 854 explicit negatives, with zero uncovered candidates. The pre-frozen profiles contained 169 judgments in total; 10 were not retrieved by either model and therefore contribute to Recall@50 rather than the union size. No parameters or gates were changed after the run.

| Gate | Hybrid delta vs. TF-IDF | Result |
| --- | ---: | --- |
| NDCG@10 | +0.134922 | Pass (`>= +0.01`) |
| MRR@10 | +0.133061 | Pass (no regression) |
| Recall@50 | +0.028571 | Pass (no regression) |
| Success@1 | +0.142857 | Pass (no regression) |
| Explicit negatives@10 | -22 | Pass (lower is better) |
| AND routing invariant | 0 ranking mismatches across 5 queries | Pass |
| Per-class NDCG@10 | `exact_identifier`: -0.010852 | **Fail** (no class regression allowed) |
